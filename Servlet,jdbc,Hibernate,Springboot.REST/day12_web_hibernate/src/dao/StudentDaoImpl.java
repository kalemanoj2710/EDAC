...
public class StudentDaoImpl implements IStudentDao {
	...
	public String admitStudent(Student student, String courseTitle) {
		...
		try {
			...
			// c : PERSISTENT
			// If you update the state of the persistent entity : hibernate auto propagates
			// changes to DB
	...
	@Override
	public String admitStudentWithCompleteDetails(Student student, int courseId) {
		String mesg = "student admission failed...";
		Session session = getSf().getCurrentSession();
		// begin tx
		Transaction tx = session.beginTransaction();
	...
	@Override
	public List<Student> getStudentDetailsByCourseName(String courseName) {
		List<Student> students = null;
		String jpql = "select s from Student s join s.selectedCourse where s.selectedCourse.title = :title";
		Session session = getSf().getCurrentSession();
		// begin tx
		Transaction tx = session.beginTransaction();
		try {
			students = session.createQuery(jpql, Student.class).setParameter("title", courseName).getResultList();
			tx.commit();
